...
def get_cell_type_abundance(imc_forSSC_raw, signatures):
    def find_first_non_hyphenated_element(elements):
        for element in elements:
            if not element.endswith('-'):
                return element
        return None  # Return None if no non-hyphenated element is found
    ...
    for celltype in (signatures.columns):
        ...
        protein_exp=imc_forSSC_raw[find_first_non_hyphenated_element(markers)]#only 1 marker used for ordering abundance of cell types
