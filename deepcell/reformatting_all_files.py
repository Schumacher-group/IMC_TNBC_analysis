...
def save_tiff_from_mcd(mcd_file,output_path,Leap_folder):
    with MCDFile(mcd_file) as f:
        for slide in f.slides:
            for acquisition in slide.acquisitions:
                ...
                filename = Leap_folder+'_'+acquisition.metadata['ID']+'.tiff'
...
def main():
    ...
    for mcd_file,main_folder in tqdm(list(zip(mcd_files_list,data_folder))):
        ...
        save_tiff_from_mcd(mcd_file,output_path,Leap_folder)
    acquisition_metadata = pd.concat(acquisition_metadatas, copy=False)
    acquisition_metadata.to_csv(path0 +"/acquisition_metadata.csv")
